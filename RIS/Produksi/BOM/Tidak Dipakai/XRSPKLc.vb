...
Public Class XRSPKLc
    ...
    Public Sub InitializeData(ByVal Bind As Collection)
        cmsl = New SqlDataAdapter("Select D.Urut,D.Nama as Divisi, BD.KompID, K.Nama As Komponen, BD.BBID, B.Nama As Bahan, BD.UkBB, BD.Sat, Round(Std,2) as Std,Keb,BD.Uk,BD.Ket,stsAdd,BBIDInd From T_BOMDtl BD Inner Join M_Div D On BD.DivID=D.DivID Inner Join M_Komp K On BD.KompID=K.KompID Inner Join M_BB B On BD.BBID=B.BBID Where BD.BOMID='" & Bind.Item("Kode").ToString & "' and ArtCode In (Select ArtCode From T_BOMPO where BOMID='" & Bind.Item("Kode").ToString & "') and SPK='True' Order By D.Urut,K.Urut,K.Nama,B.Nama,BD.Uk,BBIDInd Asc", koneksi)
