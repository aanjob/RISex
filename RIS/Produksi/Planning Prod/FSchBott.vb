...
Public Class FSchBott
    ...
    Public Sub FillDt()
        ...
        cmsl = New SqlDataAdapter("Select convert(bit,'FALSE') as Cek,(Select Max(SchIDD) From T_SchPrBott Where BOMID=SB.BOMID) As MaxSch,SchIDD, SB.BOMID,ArtName,Warna,TotPsg+TotPsgPol As TotPsg,TgtBott,Case when ETABott is null Then 'True' Else 'False' End As CekBott,ETABott,Case when KetBott='' Then 'True' Else 'False' End As CekKet,KetBott,Case when TrmBott is null Then 'True' Else 'False' End As CekTBott,TrmBott, SB.InsDate,SB.InsBy,SB.UpdDate,SB.UpdBy,convert(bit,'FALSE') As Baru From T_BOM B Inner Join T_SchPrBott SB On B.BOMID=SB.BOMID Where SB.BOMID In (Select BOMID From T_BOM where stsApp='True' and stsLunas='False') Union All Select * From (Select Top 1 convert(bit,'FALSE') as Cek,ROW_NUMBER() over (ORDER BY SP.BOMID)*-1 as MaxSch,ROW_NUMBER() over (ORDER BY SP.BOMID)*-1 as SchIDD,SP.BOMID,ArtName,Warna,TotPsg+TotPsgPol As TotPsg,TgtBott,'True' As CekBott,null as ETABott,'True' As CekKet, '' as KetBott,'True' As CekTBott,null as TrmBott,null as InsDate,null as InsBy,null as UpdDate,null as UpdBy,convert(bit,'True') As Baru From T_BOM B Left Outer Join T_SchPrPPIC SP On B.BOMID=SP.BOMID Where B.BOMID In (Select BOMID From T_SchPrPPIC Where BOMID Not In (Select BOMID From T_SchPrBott)) Order By SP.SchIDD desc)as x", koneksi)

        cmsl.TableMappings.Add("Table", "T_SchPrBott")
        Try
            DsMaster.Tables("T_SchPrBott").Clear()
        Catch ex As Exception

        End Try
        cmsl.Fill(DsMaster, "T_SchPrBott")

        Me.GridControl1.DataSource = DsMaster
        Me.GridControl1.DataMember = "T_SchPrBott"
            ...
            If Me.GridView1.GetRowCellValue(i, "Cek") = True Then
                If Me.GridView1.GetRowCellValue(i, "SchIDD") < 0 Then
                    If Not IsDBNull(Me.GridView1.GetRowCellValue(i, "BOMID")) Then
                        Dim cmSPDtl As New SqlCommand("SPInsT_SchPrBott")
                        ...
                        With cmSPDtl
                            ...
                            .Parameters.Add("@Ket", SqlDbType.VarChar).Value = Me.GridView1.GetRowCellValue(i, "KetBott")
                ...
                Else
                    ...
                    If Not IsDBNull(Me.GridView1.GetRowCellValue(i, "BOMID")) Then
                        Dim cmSPDtl As New SqlCommand("SPUpT_SchPrBott")
                        cmSPDtl.CommandType = CommandType.StoredProcedure
    ...
    Private Sub GridView1_FocusedColumnChanged(sender As Object, e As DevExpress.XtraGrid.Views.Base.FocusedColumnChangedEventArgs) Handles GridView1.FocusedColumnChanged
        If GridView1.GetRowCellValue(Me.GridView1.FocusedRowHandle, "Cek") = True Then
            ...
            If Me.GridView1.GetRowCellValue(Me.GridView1.FocusedRowHandle, "MaxSch") = Me.GridView1.GetRowCellValue(Me.GridView1.FocusedRowHandle, "SchIDD") Then
                ...
                Me.GridView1.Columns("KetBott").OptionsColumn.AllowEdit = Me.GridView1.GetRowCellValue(Me.GridView1.FocusedRowHandle, "CekKet")

            Else
                Me.GridView1.Columns("ETABott").OptionsColumn.AllowEdit = False
                Me.GridView1.Columns("TrmBott").OptionsColumn.AllowEdit = False
                Me.GridView1.Columns("KetBott").OptionsColumn.AllowEdit = False
            End If
        Else
            Me.GridView1.Columns("ETABott").OptionsColumn.AllowEdit = False
            Me.GridView1.Columns("TrmBott").OptionsColumn.AllowEdit = False
    ...
    Private Sub GridView1_InitNewRow(sender As Object, e As DevExpress.XtraGrid.Views.Grid.InitNewRowEventArgs) Handles GridView1.InitNewRow
        Try
            ...
            Me.GridView1.SetRowCellValue(e.RowHandle, "KetBott", "")
    ...
    Private Sub BEdBOMID_ButtonClick(sender As Object, e As Controls.ButtonPressedEventArgs) Handles BEdBOMID.ButtonClick
        ...
        If Not IsDBNull(dataTrans.Item("Kode").ToString) Then
            ...
            Dim command As New SqlCommand("Select Top 1 (Select Top 1 TgtBott From T_SchPrPPIC Where BOMID='" & dataTrans.Item("Kode").ToString & "' Order By SchIDD desc) as TgtBott,ETABott,Isnull(KetBott,'') As Ket,TrmBott From T_SchPrBott SB Right Outer Join T_SchPrPPIC SC On SB.BOMID=SC.BOMID and SB.TgtBott=SC.TgtBott Where SC.BOMID='" & dataTrans.Item("Kode").ToString & "' Order By SB.SchIDD desc", koneksi)
                ...
                If Reader.HasRows Then
                    While Reader.Read
                        ...
                        Else
                            ...
                            Me.GridView1.SetRowCellValue(Me.GridView1.FocusedRowHandle, "ETABott", Reader.Item(1))
                            Me.GridView1.SetRowCellValue(Me.GridView1.FocusedRowHandle, "KetBott", Reader.Item(2))
                            Me.GridView1.SetRowCellValue(Me.GridView1.FocusedRowHandle, "TrmBott", Reader.Item(3))
                        End If
                    End While
                End If
                Reader.Close()
