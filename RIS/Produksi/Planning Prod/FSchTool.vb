...
Public Class FSchTool
    ...
    Public Sub FillDt()
        ...
        cmsl = New SqlDataAdapter("Select convert(bit,'FALSE') as Cek,(Select Max(SchIDD) From T_SchPrTool Where BOMID=SC.BOMID) As MaxSch,SchIDD, SC.BOMID,ArtName,Warna,TotPsg+TotPsgPol As TotPsg,TgtUpp,TgtBott,Case when ETATool is null Then 'True' Else 'False' End As CekTool,ETATool, Case when SC.KetTool='' Then 'True' Else 'False' End As CekKet,KetTool,Case when TrmTool is null Then 'True' Else 'False' End As CekTTool, TrmTool,convert(bit,'FALSE') As Baru,SC.InsDate,SC.InsBy,SC.UpdDate,SC.UpdBy From T_BOM B Inner Join T_SchPrTool SC On B.BOMID=SC.BOMID Where SC.BOMID In (Select BOMID From T_BOM where stsApp='True' and stsLunas='False') Union All Select * From (Select Top 1 convert(bit,'FALSE') as Cek,ROW_NUMBER() over (ORDER BY SP.BOMID)*-1 as MaxSch,ROW_NUMBER() over (ORDER BY SP.BOMID)*-1 as SchIDD,SP.BOMID,ArtName,Warna,TotPsg+TotPsgPol As TotPsg,TgtUpp,TgtBott,'True' As CekTool,null as ETATool,'True' As CekKet,'' as KetTool,'True' As CekTTool,null as TrmTool,convert(bit,'True') As Baru,null as InsDate,null as InsBy,null as UpdDate,null as UpdBy From T_BOM B Left Outer Join T_SchPrPPIC SP On B.BOMID=SP.BOMID Where B.BOMID In (Select BOMID From T_SchPrPPIC Where BOMID Not In (Select BOMID From T_SchPrTool)) Order By SP.SchIDD desc)as x", koneksi)

        cmsl.TableMappings.Add("Table", "T_SchPrTool")
        Try
            DsMaster.Tables("T_SchPrTool").Clear()
        Catch ex As Exception

        End Try
        cmsl.Fill(DsMaster, "T_SchPrTool")

        Me.GridControl1.DataSource = DsMaster
        Me.GridControl1.DataMember = "T_SchPrTool"
            ...
            If Me.GridView1.GetRowCellValue(i, "Cek") = True Then
                If Me.GridView1.GetRowCellValue(i, "SchIDD") < 0 Then
                    If Not IsDBNull(Me.GridView1.GetRowCellValue(i, "BOMID")) Then
                        Dim cmSPDtl As New SqlCommand("SPInsT_SchPrTool")
                        ...
                        With cmSPDtl
                            ...
                            .Parameters.Add("@ETATool", SqlDbType.Date).Value = Me.GridView1.GetRowCellValue(i, "ETATool")
                            .Parameters.Add("@TrmTool", SqlDbType.Date).Value = Me.GridView1.GetRowCellValue(i, "TrmTool")
                            .Parameters.Add("@Ket", SqlDbType.VarChar).Value = Me.GridView1.GetRowCellValue(i, "KetTool")
                ...
                Else
                    ...
                    If Not IsDBNull(Me.GridView1.GetRowCellValue(i, "BOMID")) Then
                        Dim cmSPDtl As New SqlCommand("SPUpT_SchPrTool")
                        cmSPDtl.CommandType = CommandType.StoredProcedure
    ...
    Private Sub GridView1_FocusedColumnChanged(sender As Object, e As DevExpress.XtraGrid.Views.Base.FocusedColumnChangedEventArgs) Handles GridView1.FocusedColumnChanged
        If GridView1.GetRowCellValue(Me.GridView1.FocusedRowHandle, "Cek") = True Then
            ...
            If Me.GridView1.GetRowCellValue(Me.GridView1.FocusedRowHandle, "MaxSch") = Me.GridView1.GetRowCellValue(Me.GridView1.FocusedRowHandle, "SchIDD") Then
                Me.GridView1.Columns("ETATool").OptionsColumn.AllowEdit = Me.GridView1.GetRowCellValue(Me.GridView1.FocusedRowHandle, "CekTool")
                Me.GridView1.Columns("TrmTool").OptionsColumn.AllowEdit = Me.GridView1.GetRowCellValue(Me.GridView1.FocusedRowHandle, "CekTTool")
                Me.GridView1.Columns("KetTool").OptionsColumn.AllowEdit = Me.GridView1.GetRowCellValue(Me.GridView1.FocusedRowHandle, "CekKet")
            Else
                Me.GridView1.Columns("ETATool").OptionsColumn.AllowEdit = False
                Me.GridView1.Columns("TrmTool").OptionsColumn.AllowEdit = False
                Me.GridView1.Columns("KetTool").OptionsColumn.AllowEdit = False
            End If
        Else
            Me.GridView1.Columns("ETATool").OptionsColumn.AllowEdit = False
            Me.GridView1.Columns("TrmTool").OptionsColumn.AllowEdit = False
    ...
    Private Sub GridView1_InitNewRow(sender As Object, e As DevExpress.XtraGrid.Views.Grid.InitNewRowEventArgs) Handles GridView1.InitNewRow
        Try
            ...
            Me.GridView1.SetRowCellValue(e.RowHandle, "KetTool", "")
            Me.GridView1.SetRowCellValue(e.RowHandle, "Cek", True)
            Me.GridView1.SetRowCellValue(e.RowHandle, "Baru", True)
            Me.GridView1.SetRowCellValue(e.RowHandle, "CekTool", True)
            Me.GridView1.SetRowCellValue(e.RowHandle, "CekTTool", True)
    ...
    Private Sub BEdBOMID_ButtonClick(sender As Object, e As Controls.ButtonPressedEventArgs) Handles BEdBOMID.ButtonClick
        ...
        If Not IsDBNull(dataTrans.Item("Kode").ToString) Then
            ...
            Dim command As New SqlCommand("Select Top 1 (Select Top 1 TgtUpp From T_SchPrPPIC Where BOMID='" & dataTrans.Item("Kode").ToString & "' Order By SchIDD desc) as TgtUpp,(Select Top 1 TgtBott From T_SchPrPPIC Where BOMID='" & dataTrans.Item("Kode").ToString & "' Order By SchIDD desc) as TgtBott,ETATool,Isnull(KetTool,'') AS Ket,TrmTool From T_SchPrTool ST Right Outer Join T_SchPrPPIC SC On ST.BOMID=SC.BOMID and ST.TgtUpp=SC.TgtUpp and ST.TgtBott=SC.TgtBott Where SC.BOMID='" & dataTrans.Item("Kode").ToString & "' Order By ST.SchIDD desc", koneksi)
                ...
                If Reader.HasRows Then
                    While Reader.Read
                        ...
                        Else
                            ...
                            Me.GridView1.SetRowCellValue(Me.GridView1.FocusedRowHandle, "ETATool", Reader.Item(2))
                            Me.GridView1.SetRowCellValue(Me.GridView1.FocusedRowHandle, "KetTool", Reader.Item(3))
                            Me.GridView1.SetRowCellValue(Me.GridView1.FocusedRowHandle, "TrmTool", Reader.Item(4))
                        End If
                    End While
                End If
                Reader.Close()
