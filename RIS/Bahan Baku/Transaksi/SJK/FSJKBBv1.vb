...
Public Class FSJKBBv1
    ...
    Private Sub BVBEdit_ItemClick(sender As Object, e As DevExpress.XtraBars.Ribbon.BackstageViewItemEventArgs) Handles BVBEdit.ItemClick
        ...
        If Me.CBOJenis.EditValue = "Penjasaan Bahan" Then
            ...
            cmsl = New SqlDataAdapter("Select POID As DocID, SuppID As SCID,0 As JT,'' As TipePPn,'False' As stsPPn,MtUang,0 As DiscP,0 As DiscRpSat From T_POBB Where Tipe='Jasa' and stsKirim='False' and stsBatal='False'  or POID='" & Me.GridView2.GetFocusedDataRow.Item("DocID") & "'", koneksi)
        ...
        ElseIf Me.CBOJenis.EditValue = "Penjualan Bahan" Or Me.CBOJenis.EditValue = "Penjualan Jasa" Then
            ...
            cmsl = New SqlDataAdapter("Select SOID As DocID, S.CustID As SCID,JT,TipePPn,stsPPn,MtUang,DiscP,Round(DiscRp/TotQty,4) As DiscRpSat From T_SOBB S Inner Join M_Cust C On S.CustID=C.CustID Where stsKirim='False' and stsBatal='False' or SOID='" & Me.GridView2.GetFocusedDataRow.Item("DocID") & "'", koneksi)
            cmsl.TableMappings.Add("Table", "DocSJK")
            cmsl.Fill(DsMaster, "DocSJK")
            DsMaster.Tables("DocSJK").Clear()
            cmsl.Fill(DsMaster, "DocSJK")
    ...
    Private Sub CBOJenis_Leave(sender As Object, e As EventArgs) Handles CBOJenis.Leave
        If Me.CBOJenis.Properties.ReadOnly = False Then
            ...
            If Me.CBOJenis.EditValue = "Penjasaan Bahan" Then
                ...
                cmsl = New SqlDataAdapter("Select POID As DocID,SuppID As SCID,0 As JT,'' As TipePPn,'False' As stsPPn,MtUang,0 As DiscP,0 As DiscRpSat,Ket From T_POBB Where Tipe='Jasa' and stsKirim='False' and stsBatal='False' and Gol='" & Gol & "' or POID='" & Me.SLUDocID.EditValue & "'", koneksi)
                cmsl.TableMappings.Add("Table", "DocSJK")
                cmsl.Fill(DsMaster, "DocSJK")
                DsMaster.Tables("DocSJK").Clear()
                cmsl.Fill(DsMaster, "DocSJK")
            ...
            ElseIf Me.CBOJenis.EditValue = "Penjualan Bahan" Or Me.CBOJenis.EditValue = "Penjualan Jasa" Then
                ...
                cmsl = New SqlDataAdapter("Select SOID As DocID,S.CustID As SCID,JT,TipePPn,stsPPn,MtUang,DiscP,Round(DiscRp/TotQty,4) As DiscRpSat,S.Ket  From T_SOBB S Inner Join M_Cust C On S.CustID=C.CustID Where Jenis='" & Me.CBOJenis.EditValue & "' and stsKirim='False' and stsBatal='False' and Gol='" & Gol & "' or SOID='" & Me.SLUDocID.EditValue & "'", koneksi)
                cmsl.TableMappings.Add("Table", "DocSJK")
                cmsl.Fill(DsMaster, "DocSJK")
                DsMaster.Tables("DocSJK").Clear()
                cmsl.Fill(DsMaster, "DocSJK")
        ...
        If Not IsDBNull(Me.SLUDocID.EditValue) And Me.SLUDocID.Properties.ReadOnly = False Then
            Try
                ...
                If Me.CBOJenis.EditValue = "Penjasaan Bahan" Then
                    ...
                    Dim cmsl As SqlDataAdapter
                    cmsl = New SqlDataAdapter("Select * From(Select ROW_NUMBER() over (ORDER BY D.BBID)*-1 as SJKIDD,0 As DocIDD,'" & InisialBC & "'+D.BBID as BBID,B.Nama As Bahan,D.Sat,Case When Sum(D.Qty)-(Select Isnull((Select Sum(Qty) From T_SJKBB H1 Inner Join T_SJKBBDtl D1 On H1.SJKID=D1.SJKID Where H1.DocID='" & Me.SLUDocID.EditValue & "' and H1.SJKID <>'" & Me.TBKode.EditValue & "'),0)) > dbo.fcStokBB(D.BBID,'" & Me.SLUGd.EditValue & "','" & Me.DTPTanggal.EditValue & "','" & Me.SLUDocID.EditValue & "') Then dbo.fcStokBB(D.BBID,'" & Me.SLUGd.EditValue & "','" & Me.DTPTanggal.EditValue & "','" & Me.SLUDocID.EditValue & "') Else  Sum(D.Qty) -(Select Isnull((Select Sum(Qty) From T_SJKBB H1 Inner Join T_SJKBBDtl D1 On H1.SJKID=D1.SJKID Where H1.DocID='" & Me.SLUDocID.EditValue & "' and H1.SJKID <>'" & Me.TBKode.EditValue & "'),0)) End As Qty,0 As HarSat,0 as DiscP,0 As DiscRp,0 As DiscRpSat,0 As HarSbDisc,0 As RpDiscP,0 as HarAkhir From T_POBB H Inner Join T_POBBJs D On H.POID=D.POID Inner Join M_BB B On D.BBID=B.BBID Where H.POID='" & Me.SLUDocID.EditValue & "' Group By D.BBID,B.Nama,D.Sat)As x Where X.Qty>0", koneksi)
