...
Public Class FSJKBB
    ...
    Public Sub FillDtl(Kode As String)
        Dim cmsl As SqlDataAdapter
        cmsl = New SqlDataAdapter("Select SJKIDD,SJKID,DocIDD,(Select Isnull((Select BOMID From T_SOBBDtl Where SOID='" & Kode & "' and DocIDD=D.DocIDD),'')) as BOMID,'" & InisialBC & "'+D.BBID as BBID,BtNum,B.Nama as Bahan,D.Sat,Qty,SisaKirim,stsKirim,HarSat,HarSbDisc, DiscRpSat,DiscRp,DiscP,RpDiscP,HarAkhir From T_SJKBBDtl D Inner Join M_BB B On D.BBID=B.BBID Where SJKID='" & Kode & "'", koneksi)
    ...
    Public Sub FillDt()
        Dim cmsl As SqlDataAdapter
        cmsl = New SqlDataAdapter("Select stsApp,SJKID,PeriodID,CodeID,Tanggal,H.Gol,JenisSJ,DocID,H.GdID,G.Nama as Gudang,TjID,Case When JenisSJ='Penjasaan Bahan' Then (Select Nama From M_Supp Where SuppID=H.TjID) When JenisSJ='Penjualan Bahan' Or JenisSJ='Penjualan Jasa' Then (Select Nama From M_Cust Where CustID=H.TjID) Else (Select Nama From M_Gudang Where GdID=H.TjID) End As Tujuan,Case When JenisSJ='Penjasaan Bahan' Then (Select Alamat From M_Supp Where SuppID=H.TjID) When JenisSJ='Penjualan Bahan' Or JenisSJ='Penjualan Jasa' Then (Select Alamat From M_Cust Where CustID=H.TjID) Else (Select Alamat From M_Gudang Where GdID=H.TjID) End As Alamat,H.Ket, H.InsDate,H.InsBy,H.UpdDate, H.UpdBy,Case When JenisSJ='Penjasaan Bahan' Then (Select POCust From T_SOBB Where SOID=H.DocID) Else '' End as POCust,CurrID,MtUang,NilTukarRp,TipePPn,PersenPPn,TotQty,TotSbDisc,DiscP,RpDiscP,DiscRpSat,DiscRp,TotDisc,TotDPP,TotPPn,TotAkhir, stsPPn From T_SJKBB H Inner Join M_Gudang G On H.GdID=G.GdID Where PeriodID=" & MainModule.periodAktif & " and H.Gol ='" & Gol & "' Order By Tanggal,SJKID asc", koneksi)
        ...
        If MainModule.NoHarga = True Then
            ...
            Me.GridColumn57.Visible = False
            Me.GridColumn58.Visible = False
            Me.GridColumn59.Visible = False
    ...
    Private Sub BVTSJK_s_ItemPressed(sender As Object, e As DevExpress.XtraBars.Ribbon.BackstageViewItemEventArgs) Handles BVTSJK_s.ItemPressed
        ...
        Me.BVBApprove.Enabled = CType(TcodeCollection.Item("SJKBApr"), Boolean)
    ...
    Private Sub BVBEdit_ItemClick(sender As Object, e As DevExpress.XtraBars.Ribbon.BackstageViewItemEventArgs) Handles BVBEdit.ItemClick
        ...
        If SlCek("T_SJKBB", "stsApp", "SJKID", Me.GridView2.GetFocusedDataRow.Item("SJKID")) = True Then
        ...
        If Me.CBOJenis.EditValue = "Penjasaan Bahan" Then
            ...
            cmsl = New SqlDataAdapter("Select POID As DocID, SuppID As SCID,0 As JT,'' As TipePPn,0 As PersenPPn,'False' As stsPPn,MtUang,0 As DiscP,0 As DiscRpSat From T_POBB Where Tipe='Jasa' and stsKirim='False' and stsBatal='False'  or POID='" & Me.GridView2.GetFocusedDataRow.Item("DocID") & "'", koneksi)
            cmsl.TableMappings.Add("Table", "DocSJK")
            cmsl.Fill(DsMaster, "DocSJK")
            DsMaster.Tables("DocSJK").Clear()
            cmsl.Fill(DsMaster, "DocSJK")

            Me.SLUDocID.Properties.DataSource = DsMaster.Tables("DocSJK")
        ...
        ElseIf Me.CBOJenis.EditValue = "Penjualan Bahan" Or Me.CBOJenis.EditValue = "Penjualan Jasa" Then
            ...
            cmsl = New SqlDataAdapter("Select SOID As DocID, S.CustID As SCID,JT,TipePPn,PersenPPn,stsPPn,MtUang,DiscP,Round(DiscRp/TotQty,4) As DiscRpSat From T_SOBB S Inner Join M_Cust C On S.CustID=C.CustID Where stsKirim='False' and stsBatal='False' or SOID='" & Me.GridView2.GetFocusedDataRow.Item("DocID") & "'", koneksi)
            cmsl.TableMappings.Add("Table", "DocSJK")
            cmsl.Fill(DsMaster, "DocSJK")
            DsMaster.Tables("DocSJK").Clear()
            cmsl.Fill(DsMaster, "DocSJK")
    ...
    Private Sub BVBApprove_ItemClick(sender As Object, e As DevExpress.XtraBars.Ribbon.BackstageViewItemEventArgs) Handles BVBApprove.ItemClick
        ...
        If SlCek("T_SJKBB", "JenisSJ", "SJKID", Me.GridView2.GetFocusedDataRow.Item("SJKID")) = "Penjasaan Bahan" Then
            XtraMessageBox.Show("Data Tidak Bisa Diapprove Karena Bukan Penjualan", "Peringatan", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Exit Sub
        End If

        Dim cmSP As New SqlCommand("SPAppSJKBB")
        cmSP.CommandType = CommandType.StoredProcedure
        Dim x As Integer

        With cmSP
            .Parameters.Add("@SJKID", SqlDbType.VarChar).Value = Me.GridView2.GetFocusedDataRow.Item("SJKID")
            ...
            If Me.GridView2.GetFocusedDataRow.Item("TipePPn") <> "Non PPn" Then
                .Parameters.Add("@stsPPn", SqlDbType.VarChar).Value = "PPn"
            Else
                .Parameters.Add("@stsPPn", SqlDbType.VarChar).Value = "Non PPn"
            End If
    ...
    Private Sub BSave_Click(sender As Object, e As EventArgs) Handles BSave.Click
        ...
        Dim cmd2 As New SqlCommand("SPAftSSJKBB")
    ...
    Private Sub BEdBBID_ButtonClick(sender As Object, e As Controls.ButtonPressedEventArgs) Handles BEdBBID.ButtonClick
        Try
            If Me.CBOJenis.EditValue = "Penjasaan Bahan" And Me.SLUDocID.EditValue = "" Then
                ...
                'ElseIf Me.CBOJenis.EditValue = "Penjualan Bahan" And Me.SLUDocID.EditValue = "" Then
    ...
    Private Sub CBOJenis_Leave(sender As Object, e As EventArgs) Handles CBOJenis.Leave
        If Me.CBOJenis.Properties.ReadOnly = False Then
            ...
            If Me.CBOJenis.EditValue = "Penjasaan Bahan" Then
                ...
                cmsl = New SqlDataAdapter("Select POID As DocID,SuppID As SCID,0 As JT,'' As TipePPn,0 As PersenPPn,'False' As stsPPn,MtUang,0 As DiscP,0 As DiscRpSat,Ket From T_POBB Where Tipe='Jasa' and stsKirim='False' and stsBatal='False' and Gol='" & Gol & "' or POID='" & Me.SLUDocID.EditValue & "'", koneksi)
                cmsl.TableMappings.Add("Table", "DocSJK")
                cmsl.Fill(DsMaster, "DocSJK")
                DsMaster.Tables("DocSJK").Clear()
                cmsl.Fill(DsMaster, "DocSJK")
            ...
            ElseIf Me.CBOJenis.EditValue = "Penjualan Bahan" Or Me.CBOJenis.EditValue = "Penjualan Jasa" Then
                ...
                cmsl = New SqlDataAdapter("Select SOID As DocID,S.CustID As SCID,JT,TipePPn,PersenPPn,stsPPn,MtUang,DiscP,Round(DiscRp/TotQty,4) As DiscRpSat,S.Ket  From T_SOBB S Inner Join M_Cust C On S.CustID=C.CustID Where Jenis='" & Me.CBOJenis.EditValue & "' and stsKirim='False' and stsBatal='False' and Gol='" & Gol & "' or SOID='" & Me.SLUDocID.EditValue & "'", koneksi)
                cmsl.TableMappings.Add("Table", "DocSJK")
                cmsl.Fill(DsMaster, "DocSJK")
                DsMaster.Tables("DocSJK").Clear()
                cmsl.Fill(DsMaster, "DocSJK")
        ...
        If Not IsDBNull(Me.SLUDocID.EditValue) And Me.SLUDocID.Properties.ReadOnly = False Then
            Try
                ...
                If Me.CBOJenis.EditValue = "Penjasaan Bahan" Then
                    ...
                    Me.SLUKirim.EditValue = DsMaster.Tables("DocSJK").Select("DocID = '" & Me.SLUDocID.EditValue & "'")(0).Item("SCID")

                    Dim cmsl As SqlDataAdapter
                    cmsl = New SqlDataAdapter("Select SJKIDD,DocIDD,'' as BOMID,'" & InisialBC & "'+BBID As BBID,BtNum,Bahan,Sat,Qty,HarSat,DiscP, DiscRp,DiscRpSat,HarSbDisc,RpDiscP,HarAkhir From(Select *,Case When Keb-(Select Isnull((Select Sum(Qty) From T_SJKBB H1 Inner Join T_SJKBBDtl D1 On H1.SJKID=D1.SJKID Where H1.DocID='" & Me.SLUDocID.EditValue & "' and H1.SJKID <>'" & Me.TBKode.EditValue & "'),0)) > dbo.fcStokBB(x.BBID,'" & Me.SLUGd.EditValue & "','" & Me.DTPTanggal.EditValue & "','" & Me.TBKode.EditValue & "',BtNum) Then dbo.fcStokBB(x.BBID,'" & Me.SLUGd.EditValue & "','" & Me.DTPTanggal.EditValue & "','" & Me.TBKode.EditValue & "',BtNum) Else Keb-(Select Isnull((Select Sum(Qty) From T_SJKBB H1 Inner Join T_SJKBBDtl D1 On H1.SJKID=D1.SJKID Where H1.DocID='" & Me.SLUDocID.EditValue & "' and H1.SJKID <>'" & Me.TBKode.EditValue & "'),0)) End As Qty From(Select Distinct ROW_NUMBER() over (ORDER BY D.BBID)*-1 as SJKIDD,0 As DocIDD,'" & InisialBC & "'+D.BBID as BBID,B.Nama As Bahan,D.Sat,(Select Top 1 BtNum From(	Select BtNum,Sum(Masuk-Keluar) As Qty,Min(Tanggal) As Tanggal From T_StokBB where BBID=D.BBID and GdID='" & Me.SLUGd.EditValue & "' and Tanggal<='" & Me.DTPTanggal.EditValue & "' Group By BtNum) as x Where Qty > 0 Order By Tanggal desc) As BtNum,Sum(D.Qty) As Keb,0 As HarSat,0 as DiscP,0 As DiscRp,0 As DiscRpSat,0 As HarSbDisc,0 As RpDiscP,0 as HarAkhir From T_POBB H Inner Join T_POBBJs D On H.POID=D.POID Inner Join M_BB B On D.BBID=B.BBID Where H.POID='" & Me.SLUDocID.EditValue & "' Group By D.BBID,B.Nama,D.Sat )As x )as y Where Qty>0", koneksi)

                    'cmsl = New SqlDataAdapter("Select * From(Select ROW_NUMBER() over (ORDER BY D.BBID)*-1 as SJKIDD,0 As DocIDD,'" & InisialBC & "'+D.BBID as BBID,B.Nama As Bahan,D.Sat,Case When Sum(D.Qty)-(Select Isnull((Select Sum(Qty) From T_SJKBB H1 Inner Join T_SJKBBDtl D1 On H1.SJKID=D1.SJKID Where H1.DocID='" & Me.SLUDocID.EditValue & "' and H1.SJKID <>'" & Me.TBKode.EditValue & "'),0)) > dbo.fcStokBB(D.BBID,'" & Me.SLUGd.EditValue & "','" & Me.DTPTanggal.EditValue & "','" & Me.TBKode.EditValue & "','') Then dbo.fcStokBB(D.BBID,'" & Me.SLUGd.EditValue & "','" & Me.DTPTanggal.EditValue & "','" & Me.TBKode.EditValue & "','') Else  Sum(D.Qty) -(Select Isnull((Select Sum(Qty) From T_SJKBB H1 Inner Join T_SJKBBDtl D1 On H1.SJKID=D1.SJKID Where H1.DocID='" & Me.SLUDocID.EditValue & "' and H1.SJKID <>'" & Me.TBKode.EditValue & "'),0)) End As Qty,0 As HarSat,0 as DiscP,0 As DiscRp,0 As DiscRpSat,0 As HarSbDisc,0 As RpDiscP,0 as HarAkhir From T_POBB H Inner Join T_POBBJs D On H.POID=D.POID Inner Join M_BB B On D.BBID=B.BBID Where H.POID='" & Me.SLUDocID.EditValue & "' Group By D.BBID,B.Nama,D.Sat)As x Where X.Qty>0", koneksi)
                ...
                ElseIf Me.CBOJenis.EditValue = "Penjualan Bahan" Then
                    ...
                    Dim cmsl As SqlDataAdapter
                    cmsl = New SqlDataAdapter("Select *,Round(DiscRpSat*Qty,4) As DiscRp,Qty*HarSat As HarSbDisc,((Qty*HarSat)*DiscP)/100 As RpDiscP,(Qty*HarSat)-(((Qty*HarSat)*DiscP)/100)-(Qty*DiscRpSat) as HarAkhir From (Select ROW_NUMBER() over (ORDER BY x.BBID)*-1 as SJKIDD,BtNum,BOMID,SOID,DocIDD,BBID,Bahan,Sat,HarSat,DiscP,DiscRpSat,Case When Qty -(Select Isnull((Select Sum(Qty) From T_SJKBBDtl Where DocIDD=x.DocIDD and x.SOID='" & Me.SLUDocID.EditValue & "' and SJKID <>'" & Me.TBKode.EditValue & "'),0)) > dbo.fcStokBB(x.BBID,'" & Me.SLUGd.EditValue & "','" & Me.DTPTanggal.EditValue & "','" & Me.TBKode.EditValue & "',BtNum) Then dbo.fcStokBB(x.BBID,'" & Me.SLUGd.EditValue & "','" & Me.DTPTanggal.EditValue & "','" & Me.TBKode.EditValue & "',BtNum) Else Qty-(Select Isnull((Select Sum(Qty) From T_SJKBBDtl Where DocIDD=x.DocIDD and x.SOID='" & Me.SLUDocID.EditValue & "' and SJKID <>'" & Me.TBKode.EditValue & "'),0)) End As Qty From (Select Distinct BtNum,BOMID,H.SOID,SOIDD As DocIDD,'" & InisialBC & "'+D.BBID as BBID,B.Nama As Bahan,D.Sat,	Qty,D.HarSat,D.DiscP,Round(D.DiscRp/Qty,4) As DiscRpSat From T_SOBB H Inner Join T_SOBBDtl D On H.SOID=D.SOID Inner Join M_BB B On D.BBID=B.BBID Inner Join T_StokBB S On D.BBID=S.BBID Where H.SOID='" & Me.SLUDocID.EditValue & "') as x) as y where Qty>0", koneksi)


                    'cmsl = New SqlDataAdapter("Select *,Round(DiscRpSat*Qty,4) As DiscRp,Qty*HarSat As HarSbDisc,((Qty*HarSat)*DiscP)/100 As RpDiscP,(Qty*HarSat)-(((Qty*HarSat)*DiscP)/100)-(Qty*DiscRpSat) as HarAkhir From (Select SJKIDD,BOMID,DocIDD,BBID,BtNum,Bahan,Sat,HarSat, DiscP,DiscRpSat,Case When Qty -(Select Isnull((Select Sum(Qty) From T_SJKBBDtl Where DocIDD=x.DocIDD and x.SOID='" & Me.SLUDocID.EditValue & "' and SJKID <>'" & Me.TBKode.EditValue & "'),0)) > dbo.fcStokBB(x.BBID,'" & Me.SLUGd.EditValue & "','" & Me.DTPTanggal.EditValue & "','" & Me.TBKode.EditValue & "',BtNum) Then dbo.fcStokBB(x.BBID,'" & Me.SLUGd.EditValue & "','" & Me.DTPTanggal.EditValue & "','" & Me.TBKode.EditValue & "',BtNum) Else Qty-(Select Isnull((Select Sum(Qty) From T_SJKBBDtl Where DocIDD=x.DocIDD and x.SOID='" & Me.SLUDocID.EditValue & "' and SJKID <>'" & Me.TBKode.EditValue & "'),0)) End As Qty From(Select ROW_NUMBER() over (ORDER BY D.BBID)*-1 as SJKIDD,BOMID,H.SOID,SOIDD As DocIDD,''+D.BBID as BBID,B.Nama As Bahan,D.Sat,(Select Top 1 BtNum From(Select BtNum,Sum(Masuk-Keluar) As Qty,Min(Tanggal) As Tanggal From T_StokBB where BBID=D.BBID and GdID='" & Me.SLUGd.EditValue & "' and Tanggal<='" & Me.DTPTanggal.EditValue & "' Group By BtNum) as x Where Qty > 0 Order By Tanggal desc) AS BtNum,Qty,HarSat,D.DiscP,Round(D.DiscRp/Qty,4) As DiscRpSat From T_SOBB H Inner Join T_SOBBDtl D On H.SOID=D.SOID Inner Join M_BB B On D.BBID=B.BBID Where H.SOID='" & Me.SLUDocID.EditValue & "') as x) as y", koneksi)

                    ''cmsl = New SqlDataAdapter("Select *,Round(DiscRpSat*Qty,4) As DiscRp,Qty*HarSat As HarSbDisc,((Qty*HarSat)*DiscP)/100 As RpDiscP,(Qty*HarSat)-(((Qty*HarSat)*DiscP)/100)-(Qty*DiscRpSat) as HarAkhir From (Select ROW_NUMBER() over (ORDER BY D.BBID)*-1 as SJKIDD,BOMID,SOIDD As DocIDD,'" & InisialBC & "'+D.BBID as BBID,B.Nama As Bahan,D.Sat,Case When Qty -(Select Isnull((Select Sum(Qty) From T_SJKBBDtl Where DocIDD=D.SOIDD and H.SOID='" & Me.SLUDocID.EditValue & "' and SJKID <>'" & Me.TBKode.EditValue & "'),0)) > dbo.fcStokBB(D.BBID,'" & Me.SLUGd.EditValue & "','" & Me.DTPTanggal.EditValue & "','" & Me.TBKode.EditValue & "','') Then dbo.fcStokBB(D.BBID,'" & Me.SLUGd.EditValue & "','" & Me.DTPTanggal.EditValue & "','" & Me.TBKode.EditValue & "','') Else  Qty -(Select Isnull((Select Sum(Qty) From T_SJKBBDtl Where DocIDD=D.SOIDD and H.SOID='" & Me.SLUDocID.EditValue & "' and SJKID <>'" & Me.TBKode.EditValue & "'),0)) End As Qty,HarSat,D.DiscP,Round(D.DiscRp/Qty,4) As DiscRpSat From T_SOBB H Inner Join T_SOBBDtl D On H.SOID=D.SOID Inner Join M_BB B On D.BBID=B.BBID Where H.SOID='" & Me.SLUDocID.EditValue & "') as x", koneksi)
                ...
                ElseIf Me.CBOJenis.EditValue = "Penjualan Jasa" Then
                    ...
                    Dim cmsl As SqlDataAdapter
                    cmsl = New SqlDataAdapter("Select *,'' As BtNum,Round(DiscRpSat*Qty,4) As DiscRp,Qty*HarSat As HarSbDisc,((Qty*HarSat)*DiscP)/100 As RpDiscP,(Qty*HarSat)-(((Qty*HarSat)*DiscP)/100)-(Qty*DiscRpSat) as HarAkhir From (Select ROW_NUMBER() over (ORDER BY D.BBID)*-1 as SJKIDD,BOMID,SOIDD As DocIDD,'" & InisialBC & "'+D.BBID as BBID,B.Nama As Bahan,D.Sat,Qty -(Select Isnull((Select Sum(Qty) From T_SJKBBDtl Where DocIDD=D.SOIDD and H.SOID='" & Me.SLUDocID.EditValue & "' and SJKID <>'" & Me.TBKode.EditValue & "'),0)) As Qty,HarSat,D.DiscP,Round(D.DiscRp/Qty,4) As DiscRpSat From T_SOBB H Inner Join T_SOBBDtl D On H.SOID=D.SOID Inner Join M_BB B On D.BBID=B.BBID Where H.SOID='" & Me.SLUDocID.EditValue & "' and Qty -(Select Isnull((Select Sum(Qty) From T_SJKBBDtl Where DocIDD=D.SOIDD and H.SOID='" & Me.SLUDocID.EditValue & "' and SJKID <>'" & Me.TBKode.EditValue & "'),0))>0) as x", koneksi)
                ...
                If Me.CBOJenis.EditValue = "Penjasaan Bahan" Then
                    ...
                    Me.MKet.EditValue = ""
                Else
                    Me.RBPPn.EditValue = DsMaster.Tables("DocSJK").Select("DocID = '" & Me.SLUDocID.EditValue & "'")(0).Item("TipePPn")
                    Me.TBPersenPPn.EditValue = DsMaster.Tables("DocSJK").Select("DocID = '" & Me.SLUDocID.EditValue & "'")(0).Item("PersenPPn")
                    stsPPn = DsMaster.Tables("DocSJK").Select("DocID = '" & Me.SLUDocID.EditValue & "'")(0).Item("stsPPn")
                    MtUang = DsMaster.Tables("DocSJK").Select("DocID = '" & Me.SLUDocID.EditValue & "'")(0).Item("MtUang")
                    DueDate = DsMaster.Tables("DocSJK").Select("DocID = '" & Me.SLUDocID.EditValue & "'")(0).Item("JT")
                    Me.TBTamDiscP.EditValue = DsMaster.Tables("DocSJK").Select("DocID = '" & Me.SLUDocID.EditValue & "'")(0).Item("DiscP")
                    Me.TBDiscRpSat.EditValue = DsMaster.Tables("DocSJK").Select("DocID = '" & Me.SLUDocID.EditValue & "'")(0).Item("DiscRpSat")
                    Me.MKet.EditValue = DsMaster.Tables("DocSJK").Select("DocID = '" & Me.SLUDocID.EditValue & "'")(0).Item("Ket")
                End If
    ...
    Private Sub DTPTanggal_Leave(sender As Object, e As EventArgs) Handles DTPTanggal.Leave
        ...
        Dim i : For i = 0 To Me.GridView1.RowCount - 1
            If Me.CBOJenis.EditValue = "Penjasaan Bahan" And Me.SLUDocID.EditValue <> "" Then
                ...
                Dim command As New SqlCommand("Select Case When Sum(Qty)-(Select Isnull((Select Sum(Qty) From T_SJKBB H1 Inner Join T_SJKBBDtl D1 On H1.SJKID=D1.SJKID Where H1.DocID='" & Me.SLUDocID.EditValue & "' and H1.SJKID <>'" & Me.TBKode.EditValue & "'),0)) > dbo.fcStokBB(BBID,'" & Me.SLUGd.EditValue & "','" & Me.DTPTanggal.EditValue & "','" & Me.TBKode.EditValue & "','" & Me.GridView1.GetRowCellValue(i, "BtNum") & "') Then dbo.fcStokBB(BBID,'" & Me.SLUGd.EditValue & "','" & Me.DTPTanggal.EditValue & "','" & Me.TBKode.EditValue & "','" & Me.GridView1.GetRowCellValue(i, "BtNum") & "') Else  Sum(Qty) -(Select Isnull((Select Sum(Qty) From T_SJKBB H1 Inner Join T_SJKBBDtl D1 On H1.SJKID=D1.SJKID Where H1.DocID='" & Me.SLUDocID.EditValue & "' and H1.SJKID <>'" & Me.TBKode.EditValue & "'),0)) End From T_POBBJs Where POID='" & Me.SLUDocID.EditValue & "' and BBID='" & Me.GridView1.GetRowCellValue(i, "BBID") & "' Group By BBID", koneksi)
            ...
            ElseIf Me.CBOJenis.EditValue = "Penjualan Bahan" Then
                ...
                Dim command As New SqlCommand("Select Case When Qty -(Select Isnull((Select Sum(Qty) From T_SJKBBDtl Where DocIDD=SOIDD and SOID='" & Me.SLUDocID.EditValue & "' and SJKID <>'" & Me.TBKode.EditValue & "'),0)) > dbo.fcStokBB(BBID,'" & Me.SLUGd.EditValue & "','" & Me.DTPTanggal.EditValue & "','" & Me.TBKode.EditValue & "','" & Me.GridView1.GetRowCellValue(i, "BtNum") & "') Then dbo.fcStokBB(BBID,'" & Me.SLUGd.EditValue & "','" & Me.DTPTanggal.EditValue & "','" & Me.TBKode.EditValue & "','" & Me.GridView1.GetRowCellValue(i, "BtNum") & "') Else  Qty -(Select Isnull((Select Sum(Qty) From T_SJKBBDtl Where DocIDD=SOIDD and SOID='" & Me.SLUDocID.EditValue & "' and SJKID <>'" & Me.TBKode.EditValue & "'),0)) End From T_SOBBDtl Where SOID='" & Me.SLUDocID.EditValue & "' and SOIDD=" & Me.GridView1.GetRowCellValue(i, "DocIDD") & "", koneksi)
    ...
    Private Sub GridView1_CellValueChanged(sender As Object, e As DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs) Handles GridView1.CellValueChanged
        If e.Column Is GridView1.Columns("Qty") Then
            ...
            If Me.CBOJenis.EditValue = "Penjasaan Bahan" And Me.SLUDocID.EditValue <> "" Then
                ...
                command = New SqlCommand("Select Case When Sum(Qty)-(Select Isnull((Select Sum(Qty) From T_SJKBB H1 Inner Join T_SJKBBDtl D1 On H1.SJKID=D1.SJKID Where H1.DocID='" & Me.SLUDocID.EditValue & "' and H1.SJKID <>'" & Me.TBKode.EditValue & "'),0)) > dbo.fcStokBB(BBID,'" & Me.SLUGd.EditValue & "','" & Me.DTPTanggal.EditValue & "','" & Me.TBKode.EditValue & "','" & Me.GridView1.GetRowCellValue(e.RowHandle, "BtNum") & "') Then dbo.fcStokBB(BBID,'" & Me.SLUGd.EditValue & "','" & Me.DTPTanggal.EditValue & "','" & Me.TBKode.EditValue & "','" & Me.GridView1.GetRowCellValue(e.RowHandle, "BtNum") & "') Else  Sum(Qty)-(Select Isnull((Select Sum(Qty) From T_SJKBB H1 Inner Join T_SJKBBDtl D1 On H1.SJKID=D1.SJKID Where H1.DocID='" & Me.SLUDocID.EditValue & "' and H1.SJKID <>'" & Me.TBKode.EditValue & "'),0)) End From T_POBBJs Where POID='" & Me.SLUDocID.EditValue & "' and BBID='" & Me.GridView1.GetRowCellValue(e.RowHandle, "BBID") & "' Group By BBID", koneksi)
                ...
                command2 = New SqlCommand("Select dbo.fcStokBBBtNumAll('" & Me.GridView1.GetRowCellValue(e.RowHandle, "BBID") & "','" & Me.SLUGd.EditValue & "','" & Me.TBKode.EditValue & "'," & Me.GridView1.GetRowCellValue(e.RowHandle, "SJKIDD") & ",'" & Me.GridView1.GetRowCellValue(e.RowHandle, "BtNum") & "')", koneksi)
                ...
                Dim command3 As New SqlCommand("Select dbo.fcStokBBAll('" & Me.GridView1.GetRowCellValue(e.RowHandle, "BBID") & "','" & Me.SLUGd.EditValue & "','" & Me.TBKode.EditValue & "'," & Me.GridView1.GetRowCellValue(Me.GridView1.FocusedRowHandle, "SJKIDD") & ")", koneksi)
            ...
            ElseIf Me.CBOJenis.EditValue = "Penjasaan Bahan" And Me.SLUDocID.EditValue = "" Then
                ...
                command = New SqlCommand("Select dbo.fcStokBB('" & Me.GridView1.GetRowCellValue(e.RowHandle, "BBID") & "','" & Me.SLUGd.EditValue & "','" & Me.DTPTanggal.EditValue & "','" & Me.TBKode.EditValue & "','" & Me.GridView1.GetRowCellValue(e.RowHandle, "BtNum") & "')", koneksi)
            ...
            ElseIf Me.CBOJenis.EditValue = "Penjualan Bahan" Then
                Dim SJKLain As Decimal = 0

                Dim i : For i = 0 To Me.GridView1.RowCount - 1
                    If Me.GridView1.GetRowCellValue(i, "BBID") = Me.GridView1.GetFocusedRowCellValue("BBID") And Me.GridView1.GetRowCellValue(i, "SJKIDD") <> Me.GridView1.GetFocusedRowCellValue("SJKIDD") Then
                        SJKLain += Me.GridView1.GetRowCellValue(i, "Qty")
                    End If
                Next

                command = New SqlCommand("Select Case When Qty -(Select Isnull((Select Sum(Qty) From T_SJKBBDtl Where DocIDD=SOIDD and SOID='" & Me.SLUDocID.EditValue & "' and SJKID <>'" & Me.TBKode.EditValue & "'),0))- " & SJKLain & " > dbo.fcStokBB(BBID,'" & Me.SLUGd.EditValue & "','" & Me.DTPTanggal.EditValue & "','" & Me.TBKode.EditValue & "','" & Me.GridView1.GetRowCellValue(e.RowHandle, "BtNum") & "') Then dbo.fcStokBB(BBID,'" & Me.SLUGd.EditValue & "','" & Me.DTPTanggal.EditValue & "','" & Me.TBKode.EditValue & "','" & Me.GridView1.GetRowCellValue(e.RowHandle, "BtNum") & "') Else  Qty -(Select Isnull((Select Sum(Qty) From T_SJKBBDtl Where DocIDD=SOIDD and SOID='" & Me.SLUDocID.EditValue & "' and SJKID <>'" & Me.TBKode.EditValue & "'),0))- " & SJKLain & " End From T_SOBBDtl Where SOID='" & Me.SLUDocID.EditValue & "' and SOIDD=" & Me.GridView1.GetRowCellValue(e.RowHandle, "DocIDD") & "", koneksi)
    ...
    Private Sub GridView2_DoubleClick(sender As Object, e As EventArgs) Handles GridView2.DoubleClick
        Try
            Dim frm As New FSJKBB_d(Me.GridView2.GetFocusedDataRow.Item("JenisSJ"), Me.GridView2.GetFocusedDataRow.Item("SJKID"), Me.GridView2.GetFocusedDataRow.Item("GdID"), Manual)
            frm.ShowDialog()
            frm.Close()
        Catch ex As Exception
