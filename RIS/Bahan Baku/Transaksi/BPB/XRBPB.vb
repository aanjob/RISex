...
Public Class XRBPB
    ...
    Public Sub InitializeData(ByVal Bind As Collection, Gol As String)
        ...
        cmsl = New SqlDataAdapter("Select Bahan,Sat,QtyBOM,Kirim,CASE WHEN DocID = '' Or DocID IS NULL THEN 0 ELSE SaatIni End As SaatIni, CASE WHEN DocID = '' Or DocID IS NULL THEN 0 ELSE QtyBOM - (SaatIni + Kirim) END AS Sisa From(Select DocID,'('+ B.BBID +') ' + B.Nama as Bahan,D.Sat,Qty As Kirim,(Select Isnull ((Select Round(Sum(Keb), 0) From T_BOMDtl Where (BBID = D.BBID) And (BOMID = H.DocID)), 0) AS Expr1) AS QtyBOM,(Select Isnull ((Select Sum(T_BPBDtl.Qty) -(Select Isnull ((Select Sum(T_RPBDtl.Qty) From T_RPB INNER JOIN T_RPBDtl On T_RPB.RPBID = T_RPBDtl.RPBID Where (T_RPB.DocID = H.DocID) And (T_RPBDtl.BBID = D.BBID) And (T_RPB.Tanggal <= H.Tanggal)), 0) AS Expr1) From T_BPB INNER JOIN T_BPBDtl On T_BPB.BPBID =T_BPBDtl.BPBID Where (T_BPBDtl.BPBID <> H.BPBID) And (T_BPBDtl.BBID = D.BBID) And (T_BPB.DocID = H.DocID) And (T_BPB.Tanggal <= H.Tanggal)), 0)) AS SaatIni From T_BPB H Inner Join T_BPBDtl D On H.BPBID=D.BPBID Inner Join M_BB B On D.BBID=B.BBID Where H.BPBID ='" & Bind.Item("Kode").ToString & "') as x Order By Bahan", koneksi)

        cmsl.TableMappings.AddRange(New System.Data.Common.DataTableMapping() {New System.Data.Common.DataTableMapping("Table", "T_BPBDtl", New System.Data.Common.DataColumnMapping() {New System.Data.Common.DataColumnMapping("Bahan", "Bahan"), New System.Data.Common.DataColumnMapping("Sat", "Sat"), New System.Data.Common.DataColumnMapping("QtyBOM", "QtyBOM"), New System.Data.Common.DataColumnMapping("SaatIni", "SaatIni"), New System.Data.Common.DataColumnMapping("Kirim", "Kirim"), New System.Data.Common.DataColumnMapping("Sisa", "Sisa"), New System.Data.Common.DataColumnMapping("Sa", "Sat")})})
