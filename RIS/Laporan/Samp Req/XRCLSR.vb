...
Public Class XRCLSR
    ...
    Public Sub InitializeData(ByVal Bind As Collection)
        ...
        cmsl = New SqlDataAdapter("Select H.ReqID,H.CustID,C.Nama As Cust,D.StyleID + '/' + StlName As Style,Warna,SampType,Lastt,Uk,Qty-BtlOrder-(Select Isnull((Select Sum(Qty)+Sum(QtyRj) From T_TrmSR H1 Inner Join T_TrmSRDtl D1 On H1.TrmSRID=D1.TrmSRID Where ReqID=D.ReqID and ReqIDD=D.ReqIDD and stsApp='True'),0)) As Qty,TglKirim,H.Ket + ' ' + D.Ket As Ket From T_SampReq H Inner Join T_SampReqDtl D On H.ReqID=D.ReqID Inner Join M_Cust C On H.CustID=C.CustID Where H.CustID Like '" & Bind.Item("CustID").ToString & "' and H.ChaserID Like '" & Bind.Item("ChaserID").ToString & "' and Qty-BtlOrder-(Select Isnull((Select Sum(Qty)+Sum(QtyRj) From T_TrmSR H1 Inner Join T_TrmSRDtl D1 On H1.TrmSRID=D1.TrmSRID Where ReqID=D.ReqID and ReqIDD=D.ReqIDD and stsApp='True'),0))>0", koneksi)
