...
#End Region
    ...
    Private Sub FOmsetLkp_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        cmsl = New SqlDataAdapter("Select Cast(UserID as Varchar(20)) As UserID,Nama From M_User Where PosisiID Like '%Teknisi%'", koneksi)
        cmsl.TableMappings.Add("Table", "M_UserLUE")
        Try
            DsMaster.Tables("M_UserLUE").Clear()
        Catch ex As Exception

        End Try
        cmsl.Fill(DsMaster, "M_UserLUE")
        DsMaster.Tables("M_UserLUE").Rows.Add("%", "Semua")

        Me.SLUTeknisi.Properties.DataSource = DsMaster.Tables("M_UserLUE")
        Me.SLUTeknisi.Properties.DisplayMember = "Nama"
        Me.SLUTeknisi.Properties.ValueMember = "UserID"
    End Sub


    Private Sub BProses_Click(sender As Object, e As EventArgs) Handles BProses.Click
        Dim cmsl As SqlDataAdapter
        cmsl = New SqlDataAdapter("SPLDetCCTek", koneksi)
        ...
        cmsl.SelectCommand.Parameters.Add("@UserID", SqlDbType.VarChar).Value = Me.SLUTeknisi.EditValue

        cmsl.SelectCommand.CommandTimeout = 90000
        cmsl.TableMappings.Add("Table", "RekCCTek")

        Try
            DsLapF.Tables("RekCCTek").Clear()
        Catch ex As Exception

        End Try

        cmsl.Fill(DsLapF, "RekCCTek")

        Me.GridControl1.DataSource = DsLapF
        Me.GridControl1.DataMember = "RekCCTek"
    End Sub
    ...
    Private Sub GridView1_RowCellStyle(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Grid.RowCellStyleEventArgs) Handles GridView1.RowCellStyle
        Try
            If (e.RowHandle >= 0) Then
                If GridView1.GetRowCellValue(e.RowHandle, "Stat") = "Pending" Then
    ...
    Private Sub BExExcel_Click(sender As Object, e As EventArgs) Handles BExExcel.Click
        Dim fileName As String = ShowSaveFileDialog("Microsoft Excel Document", "Microsoft Excel|*.xls", "Rekap Check Clock Terknisi Per Tanggal " & Format(Me.DTPAkhir.EditValue, "dd-MM-yy") & "")
        If fileName <> "" Then
