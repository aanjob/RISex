...
Public Class XROutsPOBJ
    ...
    Public Sub InitializeData(ByVal Bind As Collection)
        cmsl = New SqlDataAdapter("Select Row,Rn,Rn2,POID,SuppID,Supp,Tanggal,TglKrmAw,ArtCode,ArtCode+' ('+ArtName+')' As ArtName,QtyPO,Case When Rn2=1 Then QtyBtl Else 0 End As QtyBtl,TrmID,KdPOLPB,ArtCodeLPB,SJ,TglLPB,Case When Rn=1 Then QtyLPB Else 0 End As QtyLPB,SisaQty From (Select ROW_NUMBER() OVER(ORDER BY POID Asc) AS Row,(DENSE_RANK() OVER (PARTITION BY TrmID,ArtCode ORDER BY TrmID,ArtCodeLPB asc)) As Rn,(DENSE_RANK() OVER (PARTITION BY POID,ArtCode ORDER BY POid,ArtCode,trmid asc)) As Rn2,*,QtyPO-QtyBtl-QtyLPB As SisaQty From (Select P.POID,P.SuppID, S.Nama As Supp,P.Tanggal,TglKrmAw,PD.ArtCode,B.ArtName As ArtName,sum(distinct PD.Qty) as 'QtyPO', isnull(sum(distinct PD.BtlOrder/PD.Isi),0) as 'QtyBtl',T.TrmID,T.POID as 'KdPOLPB',TD.ArtCode as 'ArtCodeLPB',T.SJ, T.Tanggal as 'TglLPB', isnull(sum(distinct TD.Qty),0) as 'QtyLPB' From T_POBJLk P inner join T_POBJLkDtl PD on P.POID = PD.POID left outer join T_TrmBJDtl TD on TD.ArtCode=PD.ArtCode and P.POID=TD.POID left outer join T_TrmBJ T on T.TrmID=TD.TrmID Inner Join M_Brg B On B.ArtCode=PD.ArtCode left outer Join M_Supp S On P.SuppID=S.SuppID Where P.POID In (" & Bind.Item("POID").ToString & ") and PD.ArtCode In (" & Bind.Item("ArtCode").ToString & ") Group by P.POID,P.SuppID,S.Nama,P.Tanggal, TglKrmAw,PD.ArtCode,B.ArtName,T.TrmID,TD.ArtCode,T.SJ,T.Tanggal,T.POID )as x ) as y order by POID asc, ArtCode asc", koneksi)
        cmsl.SelectCommand.CommandTimeout = 90000

        cmsl.TableMappings.AddRange(New System.Data.Common.DataTableMapping() {New System.Data.Common.DataTableMapping("Table", "OutsPO", New System.Data.Common.DataColumnMapping() {New System.Data.Common.DataColumnMapping("Row", "Row"), New System.Data.Common.DataColumnMapping("POID", "POID"), New System.Data.Common.DataColumnMapping("SuppID", "SuppID"), New System.Data.Common.DataColumnMapping("Supp", "Supp"), New System.Data.Common.DataColumnMapping("Tanggal", "Tanggal"), New System.Data.Common.DataColumnMapping("TglKrmAw", "TglKrmAw"), New System.Data.Common.DataColumnMapping("ArtCode", "ArtCode"), New System.Data.Common.DataColumnMapping("ArtName", "ArtName"), New System.Data.Common.DataColumnMapping("QtyPO", "QtyPO"), New System.Data.Common.DataColumnMapping("QtyBatal", "QtyBatal"), New System.Data.Common.DataColumnMapping("TrmID", "TrmID"), New System.Data.Common.DataColumnMapping("KdPOLPB", "KdPOLPB"), New System.Data.Common.DataColumnMapping("SJ", "SJ"), New System.Data.Common.DataColumnMapping("TglLPB", "TglLPB"), New System.Data.Common.DataColumnMapping("QtyLPB", "QtyLPB"), New System.Data.Common.DataColumnMapping("SisaQty", "SisaQty")})})
        ...
        Me.LBTglKirim.DataBindings.AddRange(New DevExpress.XtraReports.UI.XRBinding() {New DevExpress.XtraReports.UI.XRBinding("Text", Nothing, "OutsPO.TglKrmAw", "{0: dd/MM/yyyy}")})
        Me.LBArtName.DataBindings.AddRange(New DevExpress.XtraReports.UI.XRBinding() {New DevExpress.XtraReports.UI.XRBinding("Text", Nothing, "OutsPO.ArtName")})
        ...
        Me.LBQtyBtl.DataBindings.AddRange(New DevExpress.XtraReports.UI.XRBinding() {New DevExpress.XtraReports.UI.XRBinding("Text", Nothing, "OutsPO.QtyBtl", "{0:#,##0.##}")})
        Me.LBTrmID.DataBindings.AddRange(New DevExpress.XtraReports.UI.XRBinding() {New DevExpress.XtraReports.UI.XRBinding("Text", Nothing, "OutsPO.TrmID")})
        ...
        Me.LBTglTrm.DataBindings.AddRange(New DevExpress.XtraReports.UI.XRBinding() {New DevExpress.XtraReports.UI.XRBinding("Text", Nothing, "OutsPO.TglLPB", "{0: dd/MM/yyyy}")})
