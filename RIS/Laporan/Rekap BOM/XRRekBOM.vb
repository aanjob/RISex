...
Public Class XRRekBOM
    Dim koneksi As New SqlConnection(GlobalKoneksi)
    Dim cmsl As SqlDataAdapter

    Public Sub InitializeData(ByVal Bind As Collection)
        cmsl = New SqlDataAdapter("Select H.BOMID,ArtName,Warna,TotPsg+TotPsgPol As TotOrder,D.BBID,B.Nama,D.Sat,Sum(Keb) As Keb From T_BOM H Inner Join T_BOMDtl D On H.BOMID=D.BOMID Inner Join M_BB B On D.BBID=B.BBID Where H.BOMID In (" & Bind.Item("BOMID").ToString & ") and D.BBID In (" & Bind.Item("BBID").ToString & ") Group By H.BOMID,ArtName,Warna,TotPsg,TotPsgPol,D.BBID,B.Nama,D.Sat Order By B.Nama", koneksi)
