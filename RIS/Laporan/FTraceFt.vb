...
#End Region
    ...
    Private Sub FTraceFt_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub SLUCab_Leave(sender As Object, e As EventArgs) Handles SLUCab.Leave
        ...
        Try
            ...
            DsLapF.Tables("M_CustL" & Gol).Rows.Add("%", "Semua", "")
    ...
    Private Sub BProses_Click(sender As Object, e As EventArgs) Handles BProses.Click
        Dim cmsl As SqlDataAdapter
        cmsl = New SqlDataAdapter("Select H.JualID,H.Tanggal,S.Nama As Sales,H.JnsTrans, D.ArtCode,B.ArtName,D.SatID,D.Isi,Dos,Psg,HarSat,DiscOB, H.DiscCust,D.Ongkir,HarAkhir From T_JualBJ H Inner Join T_JualBJDtl D On H.JualID=D.JualID Inner Join M_Brg B On D.ArtCode=B.ArtCode Inner Join M_Sales S On H.SalID=S.SalID Where CustID='" & Me.SLUCust.EditValue & "' and H.Gol='" & Gol & "' Order By Tanggal,JualID desc", koneksi)
        cmsl.TableMappings.Add("Table", "T_JualBJTr" & Gol)
        cmsl.SelectCommand.CommandTimeout = 90000
        Try
            DsLapF.Tables("T_JualBJTr" & Gol).Clear()
        Catch ex As Exception

        End Try
        cmsl.Fill(DsLapF, "T_JualBJTr" & Gol)

        Me.GridControl1.DataSource = DsLapF
        Me.GridControl1.DataMember = "T_JualBJTr" & Gol
    End Sub

    Private Sub BExExcel_Click(sender As Object, e As EventArgs) Handles BExExcel.Click
        Dim fileName As String = ShowSaveFileDialog("Microsoft Excel Document", "Microsoft Excel|*.xls", "Lap Trace Faktur " & Format(Date.Today, "dd-MM-yy") & "")
        If fileName <> "" Then
