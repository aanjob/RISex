...
Public Class XXFTrmSRLama
    ...
    Public Sub FillDtl(Kode As String)
        Dim cmsl As SqlDataAdapter
        cmsl = New SqlDataAdapter("Select TrmSRIDD,TrmSRID,ReqIDD,ReqID,CustID,StlName,SampType,Uk,Material,Qty,QtyRj From T_TrmSRDtl Where TrmSRID='" & Kode & "'", koneksi)
    ...
    Private Sub BVTTrmSR_s_ItemPressed(sender As Object, e As DevExpress.XtraBars.Ribbon.BackstageViewItemEventArgs) Handles BVTTrmSR_s.ItemPressed
        ...
        Me.BVBApprove.Enabled = CType(TcodeCollection.Item("TrmSEd"), Boolean)
        Me.BVBDelete.Enabled = CType(TcodeCollection.Item("TrmSDel"), Boolean)
        Me.BVBPrint.Enabled = CType(TcodeCollection.Item("TrmSP"), Boolean)
    End Sub
    ...
    Private Sub BVBEdit_ItemClick(sender As Object, e As DevExpress.XtraBars.Ribbon.BackstageViewItemEventArgs) Handles BVBEdit.ItemClick
        ...
        Dim cmsl As SqlDataAdapter
        cmsl = New SqlDataAdapter("Select Distinct H.ReqID,C.Nama As Cust,StlName,SampType,Material,Uk From T_SampReq H Inner Join T_SampReqDtl D On H.ReqID=D.ReqID Inner Join M_Cust C On H.CustID=C.CustID Where MktID='" & Me.SLUMkt.EditValue & "' and D.stsKirim='False' and H.stsApp='True' or H.ReqID In (Select ReqID From T_SampReqDtl where H.ReqID ='" & Me.TBKode.EditValue & "') ", koneksi)
    ...
    Private Sub BVBPrint_ItemClick(sender As Object, e As DevExpress.XtraBars.Ribbon.BackstageViewItemEventArgs) Handles BVBPrint.ItemClick
        ...
        bind.Add(Me.GridView2.GetFocusedDataRow.Item("TrmSRID"), "Kode")
        bind.Add(Me.GridView2.GetFocusedDataRow.Item("Tanggal"), "Tanggal")
        bind.Add(Me.GridView2.GetFocusedDataRow.Item("Mkt"), "Mkt")
    ...
    Private Sub SLUMkt_Leave(sender As Object, e As EventArgs) Handles SLUMkt.Leave
        If Me.SLUMkt.Properties.ReadOnly = False Then
            Dim cmsl As SqlDataAdapter
            cmsl = New SqlDataAdapter("Select Distinct H.ReqID,C.Nama As Cust,StlName,SampType,Material,Uk From T_SampReq H Inner Join T_SampReqDtl D On H.ReqID=D.ReqID  Inner Join M_Cust C On H.CustID=C.CustID Where MktID='" & Me.SLUMkt.EditValue & "' and ChaserID='" & Me.SLUChaser.EditValue & "' and D.stsKirim='False' and H.stsApp='True'", koneksi)
            cmsl.TableMappings.Add("Table", "T_SampReqLUE")
            DsMaster = New System.Data.DataSet
            cmsl.Fill(DsMaster, "T_SampReqLUE")
    ...
    Private Sub BProses_Click(sender As Object, e As EventArgs) Handles BProses.Click
        ...
        cmsl = New SqlDataAdapter("Select * From (Select ROW_NUMBER() over (ORDER BY StlName)*-1 as TrmSRIDD,'" & Me.TBKode.EditValue & "' as TrmSRID,D.ReqIDD,H.ReqID,CustID,StlName,SampType,Uk,Material,Qty-(Select Isnull((Select Sum(Qty)+Sum(QtyRj) From T_TrmSRDtl Where ReqID=D.ReqID and ReqIDD=D.ReqIDD and TrmSRID<>'" & Me.TBKode.EditValue & "'),0)) As Qty, 0.0 as QtyRj From T_SampReq H Inner Join T_SampReqDtl D On H.ReqID=D.ReqID Where H.ReqID='" & Me.SLUReqID.EditValue & "') As x Where Qty>0", koneksi)
        cmsl.Fill(DsMaster, "T_TrmSRDtl")
