...
Public Class XRFtSJ
    ...
    Public Sub InitializeData(ByVal Bind As Collection)
        cmsl = New SqlDataAdapter("Select *,Concat (ArtCode,' ',Ass,' ( ',ArtName,' )') As Barang From (Select *, Case When SatID='P' Then '' Else CONCAT('(',Uk1,'-',Uk2,')') End As Ass  From (Select D.ArtCode,B.ArtName,D.SatID,(Select Min (Uk) From M_BrgAssDtl where AssID=B.AssID) As Uk1,(Select Max (Uk) From M_BrgAssDtl where AssID=B.AssID) As Uk2,HarSat,Qty,Dos,Psg,HarSbDisc,DiscOB,RpDiscOB+RpDiscCust+RpDiscL As DiscDet,HarAkhir From T_JualBJDtl D Inner Join M_Brg B On D.ArtCode=B.ArtCode Where JualID ='" & Bind.Item("JualID").ToString & "') as x Union All Select *, Case When SatID='P' Then '' Else CONCAT('(',Uk1,'-',Uk2,')') End As Ass  From (Select D.ArtCode,ArtName,D.SatID,(Select Min (Uk) From M_BrgAssDtl where AssID=B.AssID) As Uk1,(Select Max (Uk) From M_BrgAssDtl where AssID=B.AssID) As Uk2,0 As HarSat,Qty,Dos,Psg,0 As HarSbDisc,0 As DiscOB,0 As DiscDet,0 As HarAkhir From T_JualBJFree D Inner Join M_Brg B On D.ArtCode=B.ArtCode Where JualID ='" & Bind.Item("JualID").ToString & "') As x) as xyz Order By ArtCode", koneksi)
        ...
        Me.LBKode.Text = Bind.Item("JualID").ToString
