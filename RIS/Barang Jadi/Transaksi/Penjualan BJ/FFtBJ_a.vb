...
Public Class FFtBJ_a
    Dim koneksi As New SqlConnection(GlobalKoneksi)
    Public Indicator As Int16
    Dim Manual As String
    Dim Gudang, Doc As String
    Dim Tanggal As Date
    Dim Kode, PromoID, Paket, Metode, JnsCust, JnsPerhit, Kelipatan, BeliMin, JnsPot, Gol, Tampil, MtUang As String
    ...
    Public Sub New(ByVal JenisCust As String, Gol As String, Gd As String, Tgl As Date, DocID As String, Manuall As Boolean, Promo As String, Metod As String, Pakett As String, Ditampilkan As String, MataUang As String)
        ...
        Else
            ...
            Else
                ...
                If Metode = "Extra Harga Lama" Then
                    'MsgBox("Extra hrg ")
                    cmsl = New SqlDataAdapter("Select * From (Select Distinct D.ArtCode,ArtName,B.SatID,B.Isi,stsHarga,Harga,DiscOB,0 As RpDiscL,Harga-(Select Isnull(((Select Top 1 Harga From M_BrgHarga Where Jenis='" & JnsCust & "' and Gol='" & Gol & "' and MtUang ='" & MtUang & "' and ArtCode=D.ArtCode and Tanggal <H.Tanggal Order By Tanggal desc)),Harga)) As Selisih,0 As Qty,(Select Isnull((Select Sum(Masuk)-Sum(Keluar) From T_StokBJ Where ArtCode=B.ArtCode and GdID='" & Gd & "' and Tanggal <='" & Tgl & "'),0))-(Select Isnull((Select Sum(Keluar) From T_TempStokBJ Where ArtCode=B.ArtCode and GdID='" & Gd & "' and Tanggal <='" & Tgl & "'),0)) As Stok,Gol From T_PromoDtl2 D Inner Join M_Brg B On D.ArtCode=B.ArtCode Inner Join M_BrgHarga H On B.ArtCode=H.ArtCode Where Jenis='" & JnsCust & "' and PromoID='" & Promo & "' and Paket='" & Paket & "' and Gol In ('" & Gol & "','Promosi') and B.Aktif='True' and H.Aktif='True' and MtUang ='" & MtUang & "') As x Where Stok >0", koneksi)
                ...
                Else
                    'MsgBox("Else 2")
                    cmsl = New SqlDataAdapter("Select * From (Select Distinct D.ArtCode,ArtName,B.SatID,B.Isi,stsHarga,Harga,DiscOB,0 As RpDiscL,0 As Qty,0 As Selisih,(Select Isnull((Select Sum(Masuk)-Sum(Keluar) From T_StokBJ Where ArtCode=B.ArtCode and GdID='" & Gd & "' and Tanggal <='" & Tgl & "'),0))-(Select Isnull((Select Sum(Keluar) From T_TempStokBJ Where ArtCode=B.ArtCode and GdID='" & Gd & "' and Tanggal <='" & Tgl & "'),0)) As Stok,Gol From T_PromoDtl2 D Inner Join M_Brg B On D.ArtCode=B.ArtCode Inner Join M_BrgHarga H On B.ArtCode=H.ArtCode Where Jenis='" & JnsCust & "' and PromoID='" & Promo & "' and Paket='" & Paket & "' and Gol In ('" & Gol & "','Promosi') and B.Aktif='True' and H.Aktif='True' and MtUang ='" & MtUang & "') As x Where Stok >0", koneksi)
                ...
                If Metode = "Gratis Barang" Then
                    ...
                    'MsgBox("Gratis Brg ")
                    cmsl = New SqlDataAdapter("Select * From (Select Distinct D.ArtCode,ArtName,B.SatID,B.Isi,0 As Qty,(Select Isnull((Select Sum(Masuk)-Sum(Keluar) From T_StokBJ Where ArtCode=B.ArtCode and GdID='" & Gd & "' and Tanggal <='" & Tgl & "'),0))-(Select Isnull((Select Sum(Keluar) From T_TempStokBJ Where ArtCode=B.ArtCode and GdID='" & Gd & "' and Tanggal <='" & Tgl & "'),0)) As Stok,Gol From T_PromoFree D Inner Join M_Brg B On D.ArtCode=B.ArtCode  Where PromoID='" & Promo & "' and Paket='" & Paket & "' and B.Aktif='True') As x Where Stok >0", koneksi)
