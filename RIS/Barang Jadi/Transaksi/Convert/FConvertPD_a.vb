...
Public Class FConvertPD_a
    ...
    Public Sub New(Gudang As String, Tgl As Date, DocID As String, Gol As String)
        ...
        cmsl = New SqlDataAdapter("Select ArtCode,ArtName,AssID,SatID,Isi,1 as IsiDlmDos,0 As Qty,0 as Dos, 0 as Psg From M_Brg B Where B.Aktif='True' and SatID like 'D%'and Gol='" & Gol & "'", koneksi)
    ...
    Public Sub FillPsg()
        ...
        Dim x : For x = 0 To DsAddDt.Tables("M_BrgAssDtlCv").Rows.Count - 1
            Dim Stok As Integer

            Dim command As New SqlCommand("Select dbo.fcStokBJ('" & NoAssD + DsAddDt.Tables("M_BrgAssDtlCv").Rows(x).Item("Uk") & "','" & Gd & "','" & Tanggal & "','" & NoDok & "')", koneksi)
            ...
            If Stok >= DsAddDt.Tables("M_BrgAssDtlCv").Rows(x).Item("Qty") * Me.GridView1.GetFocusedDataRow.Item("Qty") Then
                Dt += 1
            End If
        Next

        If Dt = DsAddDt.Tables("M_BrgAssDtlCv").Rows.Count Then
            ...
            Next
        Else
            XtraMessageBox.Show("Stok Pasang Tidak Mencukupi", "Peringatan", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Me.GridView1.SetRowCellValue(Me.GridView1.FocusedRowHandle, "Qty", 0)
            Me.GridView1.SetRowCellValue(Me.GridView1.FocusedRowHandle, "Psg", 0)
        End If
    End Sub
    ...
    Private Sub BFinish_Click(sender As Object, e As EventArgs) Handles BFinish.Click
        ...
        Dim i : For i = 0 To GridView1.RowCount - 1
            If Me.GridView1.GetRowCellValue(i, "Qty") > 0 Then
                dataTrans2.Add(Me.GridView1.GetRowCellValue(i, "ArtCode"), "ArtCodeD" & x)
                dataTrans2.Add(Me.GridView1.GetRowCellValue(i, "ArtCode"), "ArtCode" & x)
                dataTrans2.Add(Me.GridView1.GetRowCellValue(i, "ArtName"), "ArtName" & x)
                dataTrans2.Add(Me.GridView1.GetRowCellValue(i, "SatID"), "SatID" & x)
                dataTrans2.Add(Me.GridView1.GetRowCellValue(i, "Isi"), "Isi" & x)
                dataTrans2.Add(1, "IsiDlmDos" & x)
                dataTrans2.Add(Me.GridView1.GetRowCellValue(i, "Qty"), "Qty" & x)
                dataTrans2.Add(Me.GridView1.GetRowCellValue(i, "Dos"), "Dos" & x)
                dataTrans2.Add(Me.GridView1.GetRowCellValue(i, "Psg"), "Psg" & x)

                x += 1
            End If
        Next
        dataTrans2.Add(x, "Baris")

        Dim y As Integer = 0
        Dim z : For z = 0 To GridView2.RowCount - 1
            If Me.GridView2.GetRowCellValue(z, "Qty") > 0 Then
                dataTrans.Add(Me.GridView2.GetRowCellValue(z, "ArtCodeD"), "ArtCodeD" & y)
                dataTrans.Add(Me.GridView2.GetRowCellValue(z, "ArtCode"), "ArtCode" & y)
                dataTrans.Add(Me.GridView2.GetRowCellValue(z, "ArtName"), "ArtName" & y)
                dataTrans.Add(Me.GridView2.GetRowCellValue(z, "SatID"), "SatID" & y)
                dataTrans.Add(Me.GridView2.GetRowCellValue(z, "Isi"), "Isi" & y)
                dataTrans.Add(Me.GridView2.GetRowCellValue(z, "IsiDlmDos"), "IsiDlmDos" & y)
                dataTrans.Add(Me.GridView2.GetRowCellValue(z, "Qty"), "Qty" & y)
                dataTrans.Add(Me.GridView2.GetRowCellValue(z, "Dos"), "Dos" & y)
                dataTrans.Add(Me.GridView2.GetRowCellValue(z, "Psg"), "Psg" & y)

                y += 1
            End If
        Next
        dataTrans.Add(y, "Baris")
    ...
    Private Sub GridView1_CellValueChanged(sender As Object, e As DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs) Handles GridView1.CellValueChanged
        If e.Column Is GridView1.Columns("Qty") Then
            If Me.GridView1.GetRowCellValue(e.RowHandle, "Qty") > 0 Then
