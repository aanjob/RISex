...
Public Class FPOBJJO
    Dim koneksi As New SqlConnection(GlobalKoneksi)
    Public Indicator As Int16
    Dim arrPar(-1) As String
    Dim CodeID, CurrID, JnsPPn, JnsCust As String
    ...
    Public Sub LUE()
        ...
        cmsl = New SqlDataAdapter("Select CustID,C.Nama,J.Jenis,Alamat,K.Nama As Kota From M_Cust C Inner Join M_JnsCust J On C.JnsCustID=J.JnsCustID Inner Join M_Kota K On C.KotaID=K.KotaID Where Umum='True' and C.Aktif='True'", koneksi)
    ...
    Public Sub FillDt()
        Dim cmsl As SqlDataAdapter
        cmsl = New SqlDataAdapter("Select POID,PeriodID,CodeID,Tanggal,TglKirim,TglKirimR1,TglKirimR2,TglJT,TglJTR1,TglJTR2,Jenis,H.SalID, Sl.Nama As Sales,H.SuppID,S.Nama As Supp,S.Alamat As AlamatSupp,(Select Nama From M_Kota Where KotaID=S.KotaID) As KotaSupp, H.CustID,C.Nama As Customer,C.Alamat, K.Nama As Kota,POCust,Kat,MtUang,CurrID,NilTukarRp,TipePPn,PersenPPn,TotQty,TotQtyPol,TotPsg, TotPsgPol,TotPPn,TotDPP,TotAkhir,SisaKirim, SisaProd,H.Ket,H.Grup,H.stsPPn,H.stsKirim,H.stsProd,H.stsBtlProd,H.stsBatal, H.InsDate,H.InsBy,H.UpdDate,H.UpdBy,TotPsg-(Select Isnull((Select Sum(Tot) From T_BOMPO where POID=H.POID),0))-(Select Isnull((Select Sum(BtlProd)+Sum(BtlOrder) From T_POBJJODtl where POID=H.POID),0)) As BOMPO From T_POBJJO H Inner Join M_Cust C On H.CustID=C.CustID Inner Join M_Kota K On C.KotaID=K.KotaID Left Outer Join M_Supp S On H.SuppID=S.SuppID Inner Join M_Sales Sl On H.SalID=Sl.SalID Where PeriodID=" & MainModule.periodAktif & " and H.Grup In (Select Grup From M_UsGrup Where UserID=" & MainModule.UserAktif & ") Order By Tanggal,POID asc", koneksi)
        ...
        Else
            Me.GridView1.OptionsMenu.EnableColumnMenu = True
    ...
    Private Sub BVBEdit_ItemClick(sender As Object, e As DevExpress.XtraBars.Ribbon.BackstageViewItemEventArgs) Handles BVBEdit.ItemClick
        ...
        Try
            ...
            JnsCust = DsMaster.Tables("M_CustL").Select("CustID = '" & Me.SLUCust.EditValue & "'")(0).Item("Jenis")
    ...
    Private Sub BVBRevisi_ItemClick(sender As Object, e As DevExpress.XtraBars.Ribbon.BackstageViewItemEventArgs) Handles BVBRevisi.ItemClick
        ...
        Dim frm As New FPOBJJO_r(Me.GridView2.GetFocusedDataRow.Item("POID"), TglKirim, TglJT, stsK1, stsK2, stsJ1, stsJ2, Me.GridView2.GetFocusedDataRow.Item("Tanggal"), Me.GridView2.GetFocusedDataRow.Item("TipePPn"), Me.GridView2.GetFocusedDataRow.Item("CurrID"), Me.GridView2.GetFocusedDataRow.Item("MtUang"), Me.GridView2.GetFocusedDataRow.Item("NilTukarRp"), Me.GridView2.GetFocusedDataRow.Item("POCust"))
        frm.ShowDialog()
    ...
    Private Sub BVBCancelOrder_ItemClick(sender As Object, e As DevExpress.XtraBars.Ribbon.BackstageViewItemEventArgs) Handles BVBCancelOrder.ItemClick
        ...
        If MainModule.SlBOMCancel(Me.GridView2.GetFocusedDataRow.Item("POID")) > 0 Then
            XtraMessageBox.Show("Data Tidak Bisa Dibatalkan Karena SPK Belum Dibatalkan. Silakan Hubungi PPIC Untuk Membatalkannya", "Peringatan", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Exit Sub
        End If
    ...
    Private Sub BSave_Click(sender As Object, e As EventArgs) Handles BSave.Click
        ...
        Dim cmd2 As New SqlCommand("SPAftSPOBJJO")
    ...
    Private Sub GridView2_RowCellStyle(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Grid.RowCellStyleEventArgs) Handles GridView2.RowCellStyle
        Try
            If (e.RowHandle >= 0) Then
                If Me.GridView2.GetRowCellValue(e.RowHandle, "BOMPO") > 0 And Me.GridView2.GetRowCellValue(e.RowHandle, "stsBatal") = False And Me.GridView2.GetRowCellValue(e.RowHandle, "stsBtlProd") = False Then
