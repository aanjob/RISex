...
Public Class XRSJ
    ...
    Public Sub InitializeData(ByVal Bind As Collection)
        cmsl = New SqlDataAdapter("Select *,Concat (ArtCode,' ',Ass,' ( ',ArtName,' ',Nama,' )') As Barang From (Select *, Case When SatID='P' Then '' Else CONCAT('(',Uk1,'-',Uk2,')') End As Ass  From (Select D.ArtCode,B.ArtName,W.Nama,D.SatID,(Select Min (Uk) From M_BrgAssDtl where AssID=B.AssID) As Uk1,(Select Max (Uk) From M_BrgAssDtl where AssID=B.AssID) As Uk2,Qty,Dos,Psg From T_SJDtl D Inner Join M_Brg B On D.ArtCode=B.ArtCode Inner Join M_BrgWrn W On B.WrnID=W.WrnID Where SJID ='" & Bind.Item("Kode").ToString & "') as x Union All Select *, Case When SatID='P' Then '' Else CONCAT('(',Uk1,'-',Uk2,')') End As Ass From (Select D.ArtCode,ArtName,W.Nama,D.SatID,(Select min (Uk) From M_BrgAssDtl where AssID=B.AssID) As Uk1,(Select Max (Uk) From M_BrgAssDtl where AssID=B.AssID) As Uk2,Qty,Dos,Psg From T_SJFree D Inner Join M_Brg B On D.ArtCode=B.ArtCode Inner Join M_BrgWrn W On B.WrnID=W.WrnID Where SJID ='" & Bind.Item("Kode").ToString & "') as x) as xyz Order By ArtCode", koneksi)

        cmsl.TableMappings.AddRange(New System.Data.Common.DataTableMapping() {New System.Data.Common.DataTableMapping("Table", "T_SJDtl", New System.Data.Common.DataColumnMapping() {New System.Data.Common.DataColumnMapping("Barang", "Barang"), New System.Data.Common.DataColumnMapping("Dos", "Dos"), New System.Data.Common.DataColumnMapping("Psg", "Psg")})})
        ...
        Me.LBNP.Text = " : " & Bind.Item("NPID").ToString
