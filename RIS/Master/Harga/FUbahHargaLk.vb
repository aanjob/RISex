...
Public Class FUbahHargaLk
    ...
    Private Sub LockControl()
        ...
        Me.SLUTabel.Properties.ReadOnly = True
    ...
    Private Sub OpenControl()
        ...
        Me.SLUTabel.Properties.ReadOnly = False
    ...
    Public Sub FillDt()
        ...
        cmsl = New SqlDataAdapter("Select Manuall,TblIDD,CurrID,MtUang,NilTukarRp,stsHarga,H.ArtCode,B.ArtName,H.SatID,H.Isi,Jenis,Tanggal, HargaCBP,Harga,DiscOB From M_BrgHarga H Inner Join M_Brg B On B.ArtCode=H.ArtCode Where H.Aktif='True' and Gol='" & Gol & "'", koneksi)
    ...
    Public Sub CekManual()

        If Me.CEManual.EditValue = True Then
            Me.SLUTabel.Properties.ReadOnly = True
        ...
        Else
            ...
            Me.CBOStatus.Properties.ReadOnly = True
            Me.SPIsi.Properties.ReadOnly = True
            Me.SPIsi.EditValue = 0
            Me.CBOStatus.EditValue = "Normal"

            Me.GridView2.OptionsBehavior.Editable = False

            Dim cmsl As SqlDataAdapter
            cmsl = New SqlDataAdapter("Select TblIDD,CBPBlt,AgBlt,GrBlt,RtBlt From M_TblHarga Where Aktif='True'", koneksi)

            cmsl.TableMappings.Add("Table", "M_TblHarga" & Gol)
            Try
                DsMaster.Tables("M_TblHarga" & Gol).Clear()
            Catch ex As Exception

            End Try
            cmsl.Fill(DsMaster, "M_TblHarga" & Gol)

            Me.SLUTabel.Properties.DataSource = DsMaster.Tables("M_TblHarga" & Gol)
            Me.SLUTabel.Properties.DisplayMember = "CBPBlt"
            Me.SLUTabel.Properties.ValueMember = "TblIDD"

            Dim y : For y = 0 To GridView2.RowCount - 1
                Me.GridView2.SetRowCellValue(y, "Cek", 0)
            Next
        End If
    End Sub
    ...
    Private Sub FUbahHargaLk_Activated(sender As Object, e As EventArgs) Handles Me.Activated
        CType(Me.MdiParent, FUtama).BSIInfo.Caption = "Ubah Harga Barang Jadi"
    End Sub

    Private Sub FUbahHargaLk_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        LockControl()
        Me.BVTUbahHarga_e.Selected = True
    End Sub
    ...
    Private Sub BVBEdit_ItemClick(sender As Object, e As DevExpress.XtraBars.Ribbon.BackstageViewItemEventArgs) Handles BVBEdit.ItemClick
        ...
        Indicator = "200"
        Me.DTPTanggal.Properties.ReadOnly = True
        Me.CEManual.EditValue = Me.GridView3.GetFocusedDataRow.Item("Manuall")

        CekManual()

        Me.SLUTabel.EditValue = Me.GridView3.GetFocusedDataRow.Item("TblIDD")
    ...
    Private Sub BSave_Click(sender As Object, e As EventArgs) Handles BSave.Click
        ...
        Select Case Indicator
            Case 100
                ...
                Else
                    'Input Harga Sesuai Tabel Harga

                    Dim i : For i = 0 To GridView1.RowCount - 1
                        If Me.GridView1.GetRowCellValue(i, "Cek") = True Then
                            Dim cmSP As New SqlCommand("SPInsM_BrgHargaTbl")
                            cmSP.CommandType = CommandType.StoredProcedure

                            With cmSP
                                .Parameters.Add("@Manual", SqlDbType.Bit).Value = Me.CEManual.EditValue
                                .Parameters.Add("@TblIDD", SqlDbType.Int).Value = Me.SLUTabel.EditValue
                                .Parameters.Add("@Status", SqlDbType.VarChar).Value = Me.CBOStatus.EditValue
                                .Parameters.Add("@MtUang", SqlDbType.VarChar).Value = Me.SLUMtUang.EditValue
                                .Parameters.Add("@CurrID", SqlDbType.VarChar).Value = CurrID
                                .Parameters.Add("@NilTukarRp", SqlDbType.Decimal).Value = Me.TBNilTukarRp.EditValue
            ...
            Case 200
                ...
                Else
                    ...
                    Dim i : For i = 0 To GridView1.RowCount - 1
                        If Me.GridView1.GetRowCellValue(i, "Cek") = True Then
                            Dim cmSP As New SqlCommand("SPUpM_BrgHargaTbl")
                            cmSP.CommandType = CommandType.StoredProcedure
